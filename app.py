...
with tabs[1]:
    ...
    st.markdown(
        "Demographic breakdown of your user base to identify target segments."
    )

    # Age Distribution
    st.markdown(
        "**Age Distribution**  Analyze the age profile of users and subscribers."
    )
    ...
    # Gender-Age Boxplot
    st.markdown(
        "**Age vs Gender**  Compare age ranges between genders and subscription status."
    )
    ...
    st.markdown(
        "**Subscription Rate by City**  Which cities have the highest conversion rates?"
    )
...
with tabs[2]:
    ...
    st.markdown(
        "Understand user engagement patterns and health metrics to improve product offering."
    )

    # Steps Distribution
    st.markdown(
        "**Average Daily Steps**  Higher step count often correlates with higher engagement and health awareness."
    )
    ...
    # Sleep Hours
    st.markdown(
        "**Sleep Hours Analysis**  Compare sleep habits between subscribers and non-subscribers."
    )
    ...
    # BMI by Subscription
    st.markdown(
        "**BMI Comparison**  Healthier users might be more interested in the app, let's check."
    )
    ...
    if 'Activity Level' in filtered_df.columns:
        st.markdown(
            "**Activity Level Distribution**  See if more active users are likely to subscribe."
        )
...
with tabs[3]:
    ...
    st.markdown(
        "Analyze drivers of subscription, conversion funnels, and micro-segments."
    )

    # Correlation Matrix
    st.markdown(
        "**Correlation Matrix**  Shows the relationship between numerical features and subscription status."
    )
    ...
    st.markdown(
        "**Steps vs Subscription**  Do highly active users subscribe more?"
    )
    ...
    st.markdown(
        "**User Count by City & Gender (Subscribed/Not)**"
    )
    ...
    st.markdown(
        "**BMI vs Steps: Segmentation**  Are there health-conscious clusters?"
    )
...
with tabs[4]:
    ...
    st.markdown(
        "For advanced users, explore the data with custom groupings and filters."
    )

    # Pivot Table
    st.markdown(
        "**Custom Pivot Table**  Select your own fields to group and summarize."
    )
    group_field = st.selectbox("Group By", options=['City','Gender','Age Group'] + ([ 'Activity Level'] if 'Activity Level' in filtered_df.columns else []))
    value_field = st.selectbox("Value Field", options=filtered_df.select_dtypes(include=np.number).columns)
    ...
    # Pairplot
    st.markdown(
        "**Pairwise Feature Relationships**  Explore relationships between all numeric variables."
    )
    import warnings
    warnings.filterwarnings("ignore")
    # Prevent crash if too many data points
    if len(filtered_df) <= 500:
        import seaborn as sns
        import matplotlib.pyplot as plt
        pairplot_fig = sns.pairplot(filtered_df.assign(Subscribed=filtered_df['Subscribed']), hue='Subscribed')
        st.pyplot(pairplot_fig)
    else:
        st.info("Pairplot disabled for large data (>500 rows) to keep things fast. Filter data to enable.")
